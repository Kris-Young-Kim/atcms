...
- **브랜치 전략**: Git Flow
  ...
  - main: 프로덕션 (배포)
  - develop: 개발 통합
  - feature/: 새 기능 (feature/CMS-001-client-registration, feature/CDM-007-custom-client-management, feature/SCH-001-schedule-management)
...
- **데이터 모델 변경**: PM + 개발 리더 협의
  - ⚠️ **중요**: 대상자 중심 데이터 연동 원칙 (섹션 8.5.1) 준수 필수
  - 새 서비스 모듈 추가 시 `client_id` 외래키 포함 여부 확인
- **보안 관련 변경**: 보안 검토 필수
- **성능 최적화**: 벤치마크 제시 필수
...
- **대상자 중심 데이터 연동 원칙** (섹션 8.5.1 참고) 준수 필수
- 새로운 서비스 모듈 추가 시 대상자(`clients`) 테이블과의 연동 확인
- 서비스 간 독립성 유지 (강제적인 의존성 부여 금지)
...
#### 8.5.1 대상자 중심 데이터 연동 원칙 (Client-Centric Data Integration)
...
- [ ] 새 서비스 테이블 추가 시 `client_id` 외래키 포함 여부 확인
- [ ] 대상자 삭제 시 관련 서비스 기록 처리 정책 확인 (CASCADE/RESTRICT)
- [ ] 서비스 등록 API에서 `client_id` 검증 로직 확인
- [ ] 통합 조회 API에서 해당 서비스 포함 여부 확인
- [ ] 서비스 간 강제적인 의존성 없는지 확인
...
📊 출력:
- 현재 Phase: MVP (주 8/12)
- 완료 기능: CMS-001, CMS-002, CMS-003, CMS-005, CMS-006, ERM-001, ERM-003, ERM-005 (30%)
- 진행 중 기능: CMS-004, CDM-001
- 예정 기능: CDM-007~010 (맞춤제작 요청 관리), SCH-001~005 (일정 관리), 통합 대상자 관리 기능
- 블로킹 이슈: 2개
- 테스트 커버리지: 65%
